require 'rails_helper'

RSpec.describe PostsController, type: :controller do
  let(:my_post) { Post.create!(title: RandomData.random_sentence, body: RandomData.random_paragraph) }

  #INDEX ACTION TESTS
  describe 'GET #index' do
    it 'returns http success' do
      get :index
      expect(response).to have_http_status(:success) #confirms get #index routes successfully
    end

    it 'assigns [my_post] to @posts' do
      get :index
      expect(assigns(:posts)).to eq([my_post]) #confirms that @posts returns an array containing 1 object - my_post
    end
  end


  #SHOW ACTION TESTS
  describe 'GET #show' do
    it 'returns http success' do
      get :show, params: { id: my_post.id }
      expect(response).to have_http_status(:success) #confirms get #show routes successfully for the post with id passed through params hash
    end

    it 'renders the #show view' do
      get :show, params: { id: my_post.id }
      expect(response).to render_template(:show) #confirm that the show view is rendered for the post requested
    end

    it 'assigns my_post to @post' do
      get :show, params: { id: my_post.id }
      expect(assigns(:post)).to eq(my_post) #confirm that the post showed is the post that was requested
    end
  end


  #NEW ACTION TESTS
  describe 'GET #new' do
    it 'returns http success' do
      get :new
      expect(response).to have_http_status(:success) #confirms get #new routes successfully
    end

    it 'renders the #new view' do
      get :new
      expect(response).to render_template(:new) #confirms the PostsController#new view renders successfully
    end

    it 'instantiates @post' do
      get :new
      expect(assigns(:post)).not_to be_nil #confirms that a new instance of @post is initialized
    end
  end


  #CREATE ACTION TESTS
  describe 'POST #create' do
    it 'increases the number of Post by 1' do
      expect { post :create, params: { post: { title: RandomData.random_sentence, body: RandomData.random_paragraph } } }.to change(Post, :count).by(1) #confirms the POST action successfully increases the Post object count by 1(new row in the Posts table) using the PostConrtoller#create action
    end

    it 'assigns the new post to @post' do
      post :create, params: { post: { title: RandomData.random_sentence, body: RandomData.random_paragraph } }
      expect(assigns(:post)).to eq(Post.last) #confirms the new Post object is assigned to @post
    end

    it 'redirects to the new post' do
      post :create, params: { post: { title: RandomData.random_sentence, body: RandomData.random_paragraph } }
      expect(response).to redirect_to(Post.last) #confirms our view is redirected to the newly created post
    end
  end

#  describe 'GET #edit' do
#    it 'returns http success' do
#      get :edit
#      expect(response).to have_http_status(:success)
#    end
#  end

end
